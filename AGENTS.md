...
- `/api/triage` classifies with `gemini-3.5-flash` (flash-lite fallback) and a Redis-backed flash cooldown (`triage:flash_cooldown_until`); the `triage-complaint` edge function has no cooldown, just model fallback. Both paths normalize enums then validate with Zod inside the model loop so schema failures fall back instead of 500. The edge function's Gemini `fetch` also runs inside that loop (so thrown network errors fall back too) and is bounded by a 45s `AbortController` timeout — keep both properties intact when editing the loop.
