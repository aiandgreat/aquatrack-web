...
- Both the Next.js `/api/triage` route and the `triage-complaint` Edge Function classify complaints using `gemini-3.5-flash-lite` (leveraging system instruction context caching and a 6-second timeout via `AbortController` to keep execution fast and lightweight). Enums are normalized and validated with Zod in the request loop so schema failures are caught gracefully.
