...
export default function DashboardAdmin({
  ...
  const [newAdvisoryType, setNewAdvisoryType] = useState<"warning" | "info">("warning");
  ...
  const handleUpdateRole = async (userId: string, newRole: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showFeedback("error", data.error || "Failed to update user role");
      }
    } catch (err) {
  ...
  const handleUpdateServiceAccount = async (userId: string, accountNo: string) => {
    try {
      const res = await fetch("/api/admin/users", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: userId, serviceAccountNo: accountNo }),
      });
      const data = await res.json();
      if (data.success) {
        showFeedback("success", `Service account number updated.`);
        fetchUsers();
      } else {
        showFeedback("error", data.error || "Failed to update service account");
      }
    } catch (err) {
      showFeedback("error", "Network error updating service account");
    }
  };
      ...
      <div className="flex-1 flex overflow-hidden p-[18px] gap-[18px] z-30">
        <aside className="w-64 shrink-0 bg-white border border-slate-200 rounded-[18px] p-5 shadow-sm shadow-blue-100 flex flex-col gap-1.5 overflow-y-auto">
          ...
          {[
            ...
            { key: "reports", label: "Complaints & Reports" },
            { key: "heatmaps", label: "Spatial Heatmaps" },
            { key: "telemetry", label: "IoT Telemetry" },
            { key: "analytics", label: "Water Compliance" },
            { key: "users", label: "User Profiles" },
            { key: "announcements", label: "Community Broadcasts" },
            { key: "config", label: "Simulator & Settings" },
          ].map((item) => {
        ...
        <main className="flex-1 bg-white border border-slate-200 rounded-[18px] shadow-sm shadow-blue-100 p-6 flex flex-col overflow-y-auto">
          ...
          {activeTab === "users" && (
            <UsersSection
              ...
              handleUpdateServiceAccount={handleUpdateServiceAccount}
            />
          )}
