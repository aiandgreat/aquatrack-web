  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 bg-white border border-slate-200/85 rounded-3xl p-5 space-y-5 shadow-sm">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-bold text-[#001e66]">
              ...
              <span className="text-[10px] bg-slate-150 text-slate-700 px-2 py-0.5 rounded-full font-mono">
            ...
            <input
              ...
              className="w-full h-1 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-[#001e66]"
            />
          ...
          <div className="space-y-1.5 pt-2 border-t border-slate-100">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 text-[#001e66] font-bold text-xs py-2 px-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#00aeef]/40 transition-all"
            />
            <p className="text-[10px] text-slate-400 font-medium leading-relaxed">Length of time sensor bursts reside in Upstash Redis cache.</p>
          </div>
          ...
          <div className="flex flex-col gap-2 pt-4 border-t border-slate-100">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-xs font-bold text-[#001e66] flex items-center gap-1.5">
                  <Mail className="w-4 h-4 text-[#00aeef] shrink-0" />
                  <span>Email Alert Notifications (Brevo)</span>
                </span>
                <p className="text-[9px] text-slate-400">Dispatches details via Brevo API to technicians.</p>
              </div>
              <button
                type="button"
                ...
                className={`w-12 h-6 rounded-full p-1 transition-all border-none cursor-pointer focus:outline-none ${
                  emailAlertsEnabled ? "bg-[#00aeef]" : "bg-slate-350"
                }`}
              >
          ...
          <div className={`rounded-2xl overflow-hidden border-2 transition-all duration-500 ${
            ...
            <div className={`px-4 py-3 flex items-center justify-between transition-all duration-500 ${
              ...
              <div className="flex items-center gap-2">
                <Wrench className={`w-4 h-4 shrink-0 text-white ${maintenanceMode ? "animate-spin" : ""}`} style={{ animationDuration: "6s" }} />
            ...
            <div className={`px-4 py-3.5 space-y-3 transition-all duration-500 ${
              maintenanceMode ? "bg-amber-50" : "bg-[#001e66]/5"
            }`}>
        ...
        <form onSubmit={handleTriggerSimulation} className="lg:col-span-7 bg-white border border-slate-200/85 rounded-3xl p-5 space-y-4.5 shadow-sm">
          <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
          ...
          <div className="space-y-1.5">
            <label className="text-xxs font-black text-slate-400 uppercase tracking-widest">Target Telemetry Node</label>
            <select
              value={selectedSimNodeId}
              onChange={(e) => setSelectedSimNodeId(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 hover:border-[#00aeef] text-[#001e66] font-bold text-xs py-2.5 px-3.5 rounded-xl focus:outline-none transition-all cursor-pointer"
            >
              {nodes.map((n) => (
          ...
          {/* Presets */}
          <div className="space-y-1.5">
            <label className="text-xxs font-black text-slate-400 uppercase tracking-widest">Presets</label>
                ...
                <button
                  ...
                  className={`py-2 rounded-xl text-[10px] font-black border transition-all cursor-pointer ${
                    simPreset === preset.key
                      ...
                      : "bg-slate-50 border-slate-200 hover:border-[#00aeef]/60 text-[#001e66]"
                  }`}
                >
                  {preset.label}
                </button>
          ...
          {/* Manual Inputs Grid */}
          <div className="grid grid-cols-4 gap-3 bg-slate-50/50 border border-slate-200/60 rounded-2xl p-4">
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full bg-white border border-slate-200 text-[#001e66] font-bold text-xs py-1.5 px-2 rounded-lg focus:outline-none"
              />
            </div>
            <div className="space-y-1">
          ...
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-1.5 bg-[#001e66] hover:bg-[#00aeef] text-white font-extrabold py-3.5 rounded-xl transition-all shadow-md hover:scale-[1.01] active:scale-99 border-none focus:outline-none text-xs uppercase tracking-wider cursor-pointer"
          >
